using System;
using System.Collections.Generic;
using System.Text;

namespace WojciechMikołajewicz.AdvancedListDotNetCoreMSUnitTest
{
	public class SortedListItem
	{
		public string? Key1 { get; set; }

		public int Key2 { get; set; }

		public double Key3 { get; set; }

		public decimal Key4 { get; set; }

		public short Key5 { get; set; }

		public float Key6 { get; set; }

		public long Key7 { get; set; }

		public byte Key8 { get; set; }

		public char Key9 { get; set; }

		public sbyte Key10 { get; set; }

		public DateTime Key11 { get; set; }

		public TimeSpan Key12 { get; set; }

		public ulong Key13 { get; set; }

		public uint Key14 { get; set; }

		public ushort Key15 { get; set; }

		public Guid Key16 { get; set; }

		public Version? Key17 { get; set; }
	}
}