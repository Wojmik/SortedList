...
{
	[TestClass]
	public class SortedReadOnlyList02UnitTest
	{
		const int KeysCount = 2;

		private static SortedReadOnlyList<SortedListItem, string?, int> CreateSortedReadOnlyList(IEnumerable<SortedListItem> list)
		{
			return new SortedReadOnlyList<SortedListItem, string?, int>(list, SortedReadOnlyListSampleSourceArray.AllKeysData);
		}

		private static SortedReadOnlyList<SortedListItem, string?, int> SampleList { get; set; } = default!;
