...
{
	[TestClass]
	public class SortedReadOnlyList07UnitTest
	{
		const int KeysCount = 7;

		private static SortedReadOnlyList<SortedListItem, string?, int, double, decimal, short, float, long> CreateSortedReadOnlyList(IEnumerable<SortedListItem> list)
		{
			return new SortedReadOnlyList<SortedListItem, string?, int, double, decimal, short, float, long>(list, SortedReadOnlyListSampleSourceArray.AllKeysData);
		}

		private static SortedReadOnlyList<SortedListItem, string?, int, double, decimal, short, float, long> SampleList { get; set; } = default!;
