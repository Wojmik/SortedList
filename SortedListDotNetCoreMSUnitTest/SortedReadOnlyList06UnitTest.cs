...
{
	[TestClass]
	public class SortedReadOnlyList06UnitTest
	{
		const int KeysCount = 6;

		private static SortedReadOnlyList<SortedListItem, string?, int, double, decimal, short, float> CreateSortedReadOnlyList(IEnumerable<SortedListItem> list)
		{
			return new SortedReadOnlyList<SortedListItem, string?, int, double, decimal, short, float>(list, SortedReadOnlyListSampleSourceArray.AllKeysData);
		}

		private static SortedReadOnlyList<SortedListItem, string?, int, double, decimal, short, float> SampleList { get; set; } = default!;
